...
async function calculateROI(historicalData: HistoricalDataPoint[], trade: Trade): Promise<number> {
    // Assuming historicalData is sorted by timestamp
    const entryIndex = historicalData.findIndex(point => point.timestamp === trade.entryTimestamp);
    const exitIndex = historicalData.findIndex(point => point.timestamp === trade.exitTimestamp);
